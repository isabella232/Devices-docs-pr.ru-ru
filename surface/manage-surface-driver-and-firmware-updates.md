...
- [Управление обновлениями драйвера Surface в Configuration Manager](https://docs.microsoft.com/surface/manage-surface-driver-updates-configuration-manager)
...
Драйверы Surface и обновления микропрограмм упаковываются в файлы установщика Windows (*. msi). Чтобы развернуть эти пакеты установщика Windows, можно использовать диспетчер конфигурации конечной точки или MDT. Сведения о том, как выбрать правильный MSI-файл для устройства и операционной системы, можно найти в статье рекомендации по загрузке MSI-файлов.
...
Для поддержания стабильной среды настоятельно рекомендуется сохранить четность в последней версии Windows 10.  Рекомендации по [развертыванию обновлений для Windows 10](https://docs.microsoft.com/windows/deployment/update/waas-deployment-rings-windows-10-updates)содержатся в разделе Сборка сборок.

## Загружаемые пакеты обновлений Surface

В некоторых версиях Windows 10 есть отдельные MSI-файлы, каждый из которых содержит все необходимые накопительные обновления драйверов и микропрограмм для устройств Surface. Пакеты обновления могут включать некоторые или все из следующих компонентов:
...
2. Выберите имя MSI-файла, которое соответствует модели Surface и версии Windows. Имя MSI-файла содержит минимальный поддерживаемый номер сборки Windows, необходимый для установки драйверов и встроенного по. Например, как показано на приведенном ниже рисунке, чтобы обновить книгу Surface 2 с помощью сборки 18362 в Windows 10, выберите **SurfaceBook2_Win10_18362_19.101.13994.msi.** Для Surface Book 2 с построением 16299 в Windows 10 выберите **SurfaceBook2_Win10_16299_1803509_3.msi**.
...
Так как в августе 2019. MSI-файлы использовали следующие соглашения об именовании:
