...
description: Узнайте, как управлять режимом управления microsoft Surface Enterprise (SEMM) с помощью диспетчера конфигурации конечной точки.
...
Функция Microsoft Surface Enterprise режиме управления (SEMM) устройств Surface UEFI позволяет администраторам управлять и обеспечивать безопасность конфигурации параметров Surface UEFI. Для большинства организаций этот процесс выполняется путем создания пакетов Windows установки (.msi) с помощью средства конфигурации Microsoft Surface UEFI. Затем эти пакеты запускаются или развертываются на клиентских устройствах Surface для регистрации устройств в SEMM и обновления конфигурации параметров Surface UEFI.
...
> [!Note]
> Вам также потребуется доступ к сертификату, который вы собираетесь использовать для обеспечения безопасности SEMM. Сведения о требованиях к этому сертификату см. в справке [Surface Enterprise режиме управления](surface-enterprise-management-mode.md#surface-enterprise-management-mode-certificate-requirements).
> 
...
Развертывание Microsoft Surface UEFI Manager является типичным развертыванием приложений. Файл установки UEFI Manager Microsoft Surface — это стандартный файл Windows установки, который можно установить со стандартным [тихим параметром](https://msdn.microsoft.com/library/windows/desktop/aa367988).
...
3. **Расширь** узел управления приложениями библиотеки программного обеспечения и выберите **приложения**.
...
5. Мастер создания приложений представляет ряд действий:
   ...
   * **General** . **Автоматически обнаруживаемая информация об этом** приложении из параметра файлов установки выбирается по умолчанию. В поле **Type** **Windows (.msi файл)** также выбирается по умолчанию. Выберите **Обзор** для навигации и выберите **SurfaceUEFIManagerSetup.msi**, а затем выберите **Далее**.
   
      > [!Note]
      > Расположение SurfaceUEFIManagerSetup.msi должно быть в сетевой сети и расположено в папке, которая не содержит других файлов. Расположение локального файла невозможно использовать.
...
* **ConfigureSEMM.ps1** . Используйте этот скрипт для создания пакетов конфигурации для устройств Surface с нужными настройками Surface UEFI для применения указанных параметров на устройстве Surface, регистрации устройства в SEMM и установки ключа реестра, используемого для определения регистрации устройства в SEMM.
...
Первым регионом скрипта, который необходимо изменить, является часть, которая указывает и загружает сертификат SEMM, а также указывает версию SurfaceUEFIManager, а также имена пакета конфигурации SEMM и пакета сброса SEMM. Имя сертификата и версия SurfaceUEFIManager указаны на строках от 56 до 73 в сценарии ConfigureSEMM.ps1.
...
Команда удалить SEMM с помощью ResetSEMM.ps1 будет следующим образом.
...
2. Проследовать через мастер создания приложений следующим образом:
   ...
   - Выполните следующие действия мастера типа развертывания:
     ...
     * **Содержимое** — **выберите Обзор** рядом с полем **Расположение** контента, а затем выберите папку, в которой расположены скрипты конфигурации SEMM. В поле **Программа установки** введите команду [установки,](#deploy-semm-configuration-manager-scripts) найденную ранее в этой статье. В поле **Uninstall Program** введите команду удалить[](#deploy-semm-configuration-manager-scripts), найденную ранее в этой статье (показано на рисунке 2). Выберите **Далее** , чтобы перейти на следующую страницу.
