...
- Параметры, связанные с элементами управления в оболочке Windows 10 Корпоративная, неактуальны для Surface Hub.
...
*Политики организации, которые это* может повлиять: - Политики, связанные с разрешениями доступа и владением файлами и папок, не применяются для Surface Hub. Пользователи не могут просматривать и сохранять файлы в системных каталогах и сетевых папках.
...
- Используйте рекомендации для Windows 10 Корпоративная, чтобы определить компоненты и требования к сети для приложений по умолчанию на Surface Hub.
...
Чтобы Surface Hub можно было использовать в общественных местах, таких как конференц-залы, в настраиваемой ОС устройства реализовано множество доступных в Windows10 функций безопасности и блокировки. Дополнительные сведения см. [в Surface Hub Обзор безопасности](surface-hub-security.md)
...
Административные функции Windows 10 Корпоративная, такие как консоль управления Microsoft, Run, Командная подсказка, PowerShell, редактор реестра и диспетчер задач, не поддерживаются Surface Hub. Приложение "Параметры" содержит все административные функции, которые доступны локально на Surface Hub.

#### <a name="event-viewer"></a>Зритель событий

Windows 10 для совместной работы 2020 обновление 2 добавляет поддержку Windows просмотра событий, которая идентична установленной на Windows 10 Pro или Windows 10 Корпоративная.[](/host-integration-server/core/windows-event-viewer1) 

**Чтобы открыть зрителя событий:**

1. Во входе **Параметры** приложение с учетными данными администратора.
2. Выберите **обновление &** **SecurityLogs** >  и в статье Просмотр событий, выберите **Open**. 

Дополнительные дополнительные Windows [просмотра событий](/host-integration-server/core/windows-event-viewer1).

### <a name="remote-management-and-monitoring"></a>Удаленное управление и мониторинг

Surface Hub поддерживает удаленное управление с помощью решений управления мобильными устройствами (MDM[),](/mem/intune/) таких как Microsoft Intune и мониторинга через [Azure Monitor](/azure/azure-monitor/). 
...
- Параметры групповой политики не применяются при Surface Hub к домену. Параметры политики, связанные с членством в домене, не применяются к Surface Hub.
...
Операционная система Surface Hub использует компонент Windows 10 Connected User Experience and Telemetry для сбора и передачи данных диагностики. Дополнительные сведения см. в разделе [Настройка диагностических данных Windows в организации](/windows/privacy/configure-windows-diagnostic-data-in-your-organization).

*Это может повлиять на следующие политики организации:* 

- Настройте уровни данных диагностики для Surface Hub так же, как и для Windows 10 Корпоративная.
