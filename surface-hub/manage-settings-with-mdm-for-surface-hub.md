...
- **Создание настраиваемой конфигурации профиля.**  Расширяйте область управления с помощью единого идентификатора ресурсов OMA (OMA URI) из любого из [CSPs,](/windows/client-management/mdm/configuration-service-provider-reference#surfacehubcspsupport)поддерживаемых Microsoft Surface Hub. См. [создание настраиваемой конфигурации](#create-custom-configuration-profile)профиля .
...
Дополнительные сведения о создании и управлении профилями см. в странице Ограничение возможностей устройств [с использованием политики в Microsoft Intune.](/mem/intune/configuration/device-restrictions-configure#create-the-profile)
 
Дополнительные сведения об управлении Surface Hub функциями и настройками см. в Surface Hub Windows 10 для совместной работы ограничения устройства [в Microsoft Intune](/mem/intune/configuration/device-restrictions-windows-10-teams)
...
Можно расширить область управления, [](/mem/intune/configuration/custom-settings-configure) создав настраиваемый профиль с помощью URI OMA из любого из [CSP,](/windows/client-management/mdm/configuration-service-provider-reference#surfacehubcspsupport)поддерживаемых Microsoft Surface Hub. Каждый параметр в CSP имеет соответствующий OMA-URI, который можно установить с помощью пользовательских профилей конфигурации в Intune. Дополнительные сведения о CSP, поддерживаемых Surface Hub, можно ссылаться на следующие ресурсы: 
...
> [!NOTE]
> Управление учетной записью устройства с помощью параметров [службы CSP SurfaceHub](/windows/client-management/mdm/surfacehub-csp) в настоящее время невозможно с помощью Intune и требует использования стороннего поставщика MDM.
...
1. В [документации CSP](/windows/client-management/mdm/configuration-service-provider-reference#surfacehubcspsupport)определите корневой узел CSP. Как правило, это выглядит **как ./Vendor/MSFT/NameOfCSP**. 
    - **Пример:** Корневой узел [CSP SurfaceHub](/windows/client-management/mdm/surfacehub-csp) **— ./Vendor/MSFT/SurfaceHub.**
...
- [Управление Teams определенными функциями](#manage-teams-specific-features)
...
### <a name="manage-teams-specific-features"></a>Управление Teams определенными функциями

Вы можете создать настраиваемый профиль конфигурации для управления Teams согласованными собраниями, близостью и другими функциями. Подробнее см. в Microsoft Teams [конфигурации Surface Hub.](/microsoftteams/rooms/surface-hub-manage-config)
...
Чтобы изменить установку по умолчанию, используйте [настраиваемый](/mem/intune/configuration/custom-settings-configure) профиль, чтобы Teams режим собрания следующим образом:  
